...
function generateICS(event: typeof UPCOMING[0]) {
  ...
  const a = document.createElement("a"); a.href = url; a.download = `${event.name.replace(/\s+/g, "_")}.ics`; a.click();
...
function Events() {
  ...
  function handleRegister(name: string) {
    setRegistered(prev => { const s = new Set(prev); s.add(name); return s; });
  }
    ...
    <PageShell>
      <PageHeader eyebrow="Events" title="What CaseReady is building. What we've already done.">
        <div className="flex gap-6 border-b border-border -mb-2">
          ...
          <button onClick={() => setTab("past")} className="pb-3 text-[14px] font-medium relative" style={{ color: tab === "past" ? "#E8490F" : "#5C5C5A" }}>
            Past Events {tab === "past" && <span className="absolute left-0 right-0 -bottom-px h-0.5 bg-orange" />}
          </button>
        </div>
      </PageHeader>
        ...
        <section className="bg-background">
          <div className="mx-auto max-w-[1180px] px-6 py-14">
            <div className="p-8 rounded-[16px]" style={{ background: "#FFF0EB", border: "1px solid #E8C4B0" }}>
              ...
              <p className="mt-3 text-[15px] text-text-secondary">{featured.date} · {featured.time} · {featured.venue}</p>
              ...
              <div className="mt-6 flex gap-3 flex-wrap">
                {registered.has(featured.name) ? (
                  ...
                  <button className="btn-primary" onClick={() => handleRegister(featured.name)}>Register Now →</button>
                )}
                <button className="btn-secondary" onClick={() => generateICS(featured)}>Add to Calendar</button>
              </div>
            </div>
                ...
                <article key={e.name} className="card-base p-6">
                  ...
                  <div className="mt-4 flex gap-2 flex-wrap">
                    {registered.has(e.name) ? (
                      ...
                      <button className="btn-ghost text-[13px]" onClick={() => handleRegister(e.name)}>Register →</button>
                    )}
                    <button className="text-[12px] text-text-muted hover:text-orange transition-colors" onClick={() => generateICS(e)}>📅 Calendar</button>
                  </div>
                </article>
              ))}
            </div>
