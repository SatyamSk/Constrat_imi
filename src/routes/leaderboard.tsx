      ...
      <div className="mx-auto max-w-[1180px] px-5 md:px-6 -mt-4 pb-20">
        ...
        >
          ...
          <p className="text-[12px] text-text-secondary" style={{ fontFamily: "var(--font-mono)" }}>
            (Cases × 3) + (Guesstimates × 1) + (Streak × 2) + (PI × 0.5) + (Reviews × 2) +
            (Experiences × 5)
          </p>
        </div>
          ...
          <div className="flex gap-1">
            {(["week", "month", "all"] as const).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-3 py-1.5 rounded-md text-[12px] font-medium transition-all ${period === p ? "bg-orange text-white" : "text-text-muted hover:text-text-primary"}`}
              >
            ...
            <AnimatedSection key={u.rank} delay={i * 40}>
              <div className="card-base p-4 flex items-center gap-4 hover:border-orange/30">
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="font-bold">{u.guesstimates}</p>
                    <p className="text-[9px] text-text-muted">Guess.</p>
                  </div>
                  <div className="text-center">
                    <p className="font-bold" style={{ color: "#E8490F" }}>
                      {u.streak}
                    </p>
